...
def main():
    ...
    if teacher_train:
        ...
        train_loader, test_loader = Spliter_DataLoader("hate_all.csv", "neutral_all.csv", tokenizer, batch_size=16,
                                                       test_size=0.2, random_state=38, subset_size=10000)
    ...
    for target in OUR_TARGET:
        ...
        #Fetch the data and split it into test and training
        train_loader, test_loader = Spliter_DataLoader(hate_target,neutral_target, tokenizer, batch_size=16, test_size=0.2, random_state=38, subset_size = TARGET_SUBSET_DIC[target])
        ...
        warmup_scheduler = get_linear_schedule_with_warmup(optimizer, num_warmup_steps=num_warmup_steps, num_training_steps=num_training_steps)
